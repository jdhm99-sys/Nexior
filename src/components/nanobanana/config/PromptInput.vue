<template>
  <div>
    ...
    <div style="margin-top: 12px; display: flex; gap: 8px; flex-wrap: wrap; justify-content: center;">
      <el-button size="small" type="primary" plain @click="setPresetPrompt('处理流程与核心要求：
图案识别与提取 (核心)：
...
图案完整性与比例： 图像中的单元图案整个图案必须保持完全可见，图案的任何部分都不得被图像边缘裁剪。图案单元应被缩放到尽可能大，以充分展示细节。
剥离衣物属性： 在生成过程中，完全忽略并去除原始服装的形状、接缝、褶皱、皱纹以及任何光照、阴影、反光等环境效果。最终图像必须是纯粹、平整的织物图案。
样式与颜色一致性： 生成图案的所有样式和颜色（包括所有色阶和纹理感）必须与原始图片中的图案完全相同，无任何偏差。  不要重复单元，给图案我，不要文字')">定位印花</el-button>
      <el-button size="small" type="success" plain @click="setPresetPrompt('处理流程与核心要求：
图案识别与提取 (核心)：
...
剥离衣物属性： 在生成过程中，完全忽略并去除原始服装的形状、接缝、褶皱、皱纹以及任何光照、阴影、反光等环境效果。最终图像必须是纯粹、平整的织物图案。
样式与颜色一致性： 生成图案的所有样式和颜色（包括所有色阶和纹理感）必须与原始图片中的图案完全相同，无任何偏差。  给图案我，不要文字')">布匹印花</el-button>
      <el-button size="small" type="warning" plain @click="setPresetPrompt('这是我扫描的布料图案，请对图片进行深度视觉分析，清除布料上的布纹杂点以及可能有的暗纹痕迹，重新输出超高清图片')">消除布纹</el-button>
    </div>
  </div>
</template>
